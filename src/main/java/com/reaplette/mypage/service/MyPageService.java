...
public class MyPageService {
    ...
    public void setUser(UserVO user, MultipartFile profileImagePathForm) {
        ...
        if (!profileImagePathForm.isEmpty() && user.getProfileImagePath().isEmpty()) {
            ...
            String filePath = uploadDirectory + filePrefix + "_"; // 접두사를 붙인 경로
            ...
            if (dir.exists() && dir.isDirectory()) {
                ...
                log.warn("Directory not found: " + uploadDirectory);
            }
...
//        private String CLIENT_SECRET;    // 변경 가능한 값
    ...
    public Map<String, List> getPreferenceList(String id, HttpSession session) {
        ...
        if (getPreferenceAuthorList.get(getPreferenceAuthorList.size() - 1) == null) {
            session.setAttribute("authorCategory", "아직 선호 작가가 없습니다 !");
        } else {
            int nonNullCount = 0;
            String[] pa = new String[getPreferenceAuthorList.size()];
            for (int i = getPreferenceAuthorList.size() - 1; i >= 0; i--) {
                if (getPreferenceAuthorList.get(i) == null) {
                    break;
                } else {
                    nonNullCount++;
            ...
            // nonNullCount 만큼 새로운 배열 rpa 생성
            String[] rpa = new String[nonNullCount];

            // null이 아닌 값들을 rpa 배열에 복사
            int index = 0;
            for (int i = pa.length - 1; i >= 0; i--) {
                if (pa[i] != null) {
                    rpa[index++] = pa[i]; // rpa 배열에 값 저장
                }
            }

            String[] authorResult = getMostCommonKeywords((rpa));
