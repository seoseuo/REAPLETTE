...
public class SearchService {
    ...
    public String getBookCategoryInfo(String authorName, String bookName) {
        try {
            // 입력 받은 책 제목과 작가 이름을 URL 인코딩
            String query = URLEncoder.encode(authorName + " " + bookName, StandardCharsets.UTF_8);

            // 네이버 쇼핑북 검색 URL로 요청
            String searchUrl = "https://search.shopping.naver.com/book/search?bookTabType=ALL&pageIndex=1&pageSize=40&query=" + query + "&sort=REL";
            Document doc = Jsoup.connect(searchUrl).get();

            // 검색 결과에서 책의 링크 찾기
            Elements bookLinks = doc.select(".bookListItem_item_inner__9LmpD a");
            if (bookLinks.isEmpty()) {
                return null; // 책 링크가 없으면 null 반환
            }

            // 첫 번째 책의 상세 페이지 링크 가져오기
            Element firstBookLink = bookLinks.first();
            String bookUrl = firstBookLink.attr("href");

            // 책 상세 페이지에서 카테고리 추출
            Document bookPage = Jsoup.connect(bookUrl).get();
            Elements categoryElements = bookPage.select(".bookBasicInfo_info_detail__wfrBr");  // 카테고리 선택자 예시
