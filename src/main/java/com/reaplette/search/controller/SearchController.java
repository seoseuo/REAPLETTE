...
//            }
    ...
    public ResponseEntity<Map<String, String>> bookMark(HttpSession session,
                                                        @RequestBody BookmarkVO bookmark, Model model) {
        log.info("POST : /total/book/bookMark");
        model.addAttribute("keyword", bookmark.getCategory());
        ...
        if ((UserVO) session.getAttribute("user") == null) {
            ...
            Map<String, String> response = new HashMap<>();
            response.put("redirectUrl", "/search/total/book/detail?keyword="+ bookmark.getCategory() +"&isbn=" + bookmark.getBookId());
