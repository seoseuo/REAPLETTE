...
//        log.info("콜백 호출됨: code={}, state={}, sessionState={}", code, state, session.getAttribute("naverState"));
        try {
            ...
            String idKey = (String) userInfo.get("id"); // 네이버 고유 ID
            String id = (String) session.getAttribute("id");
            ...
            if (user != null) {
                // 기존 회원
                session.setAttribute("user", user);
                log.info("기존 회원: ID={}, Key={}", id, idKey);
                return "redirect:/";
            }

            // idKey 중복 확인
            if (!loginService.isIdKeyUnique(idKey)) {
                String alertMessage = "로그인 계정이 일치하지 않습니다.\n이메일을 다시 입력하세요.";
                return "redirect:/login/enterEmail?alertMessage=" + URLEncoder.encode(alertMessage, StandardCharsets.UTF_8);
            }

            // 신규 회원
            log.info("신규 회원: ID={}, Key={}", id, idKey);
        ...
        try {
            ...
            if (isPasswordValid) {
                ...
                if (attemptCount != null && attemptCount >= 5) {
                    response.put("error", "5회 이상 잘못된 비밀번호 입력으로 로그인을 제한합니다. 1분 후 다시 시도하세요.");
                } else {
