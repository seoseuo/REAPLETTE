...
public class LoginController {
    ...
    // 비밀번호 재설정 처리
    @PostMapping(value = "/findPassword", consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE)
    @ResponseBody
    public Map<String, Object> handleFindPassword(@RequestBody Map<String, String> request, HttpSession session) {


        String id = request.get("id");
        Map<String, Object> response = new HashMap<>();

        // 세션에 ID가 없으면 에러 반환 -- 예외 처리 메시지칸 없음
        if (id == null) {
            response.put("error", "아이디 세션이 만료되었습니다. 다시 입력하세요.");
            response.put("redirect", "/login/enterEmail");
            return response;
        ...
        try {
            // 임시 비밀번호 생성 및 이메일 발송
            loginService.resetPasswordAndSendEmail(id); // ID 기반으로 비밀번호 재설정
            session.setAttribute("id", id); // 세션에 ID 저장
            response.put("success", true);
            response.put("message", "임시 비밀번호를 이메일로 발송했습니다.");
        } catch (IllegalArgumentException e) {
            logger.error("사용자 정보 확인 실패: {}", e.getMessage());
            response.put("error", "해당 사용자 정보를 찾을 수 없습니다.");
        } catch (Exception e) {
            logger.error("비밀번호 재설정 중 오류 발생: {}", e.getMessage());
