...
//    }
    ...
    // idKey의 유니크 여부 확인
    public boolean isIdKeyUnique(String idKey) {
        UserVO user = loginMapper.getUserByIdAndIdKey(null, idKey);
        return user == null; // 데이터가 없으면 유니크
    }
...
//                log.error("사용자 정보 요청 실패: {}", user.get("error_description"));
    ...
    */
    ...
    public boolean validatePassword(String id, String pw, HttpSession session) {
        final int MAX_ATTEMPTS = 5;  // 최대 로그인 시도 횟수
        final long LOCK_TIME = 60000; // 1분 잠금
        // 로그인 시도 횟수 및 제한 시간
